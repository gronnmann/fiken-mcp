...
describe('fiken_create_purchase', () => {
  it('calls POST /purchases with body', async () => {
    mockMutate.mockResolvedValue({ created: true, location: '/companies/test-slug/purchases/1' });
    const body = {
      transactionDate: '2024-01-15',
      lines: [{ description: 'Office supplies', netAmount: 50000, vat: 12500, vatType: 'HIGH', account: '6540' }],
    };
  ...
  it('returns error on failure', async () => {
    ...
    const result = await server.getHandler('fiken_create_purchase')({ transactionDate: '2024-01-01', lines: [] });
    expect(result.isError).toBe(true);
  });
});
...
describe('fiken_get_purchase_attachments', () => {
  ...
  it('returns error on failure', async () => {
    mockGet.mockRejectedValue(new Error('Fiken 404: Not Found'));
    const result = await server.getHandler('fiken_get_purchase_attachments')({ purchaseId: 999 });
    expect(result.isError).toBe(true);
  });
});
...
describe('fiken_create_purchase_draft', () => {
  it('calls POST /purchases/drafts with body', async () => {
    mockMutate.mockResolvedValue({ created: true, location: '/companies/test-slug/purchases/drafts/1' });
    ...
    expect(mockMutate).toHaveBeenCalledWith('POST', '/companies/test-slug/purchases/drafts', body);
    expect(result.content[0].text).toContain('created');
  });
...
describe('fiken_update_purchase_draft', () => {
  it('calls PUT /purchases/drafts/{draftId} with body (draftId excluded)', async () => {
    ...
    expect(mockMutate).toHaveBeenCalledWith(
      ...
      { purchaseDate: '2024-02-01', paymentAccount: '1920:10001' },
    );
    expect(result.isError).toBeUndefined();
  });
...
describe('fiken_delete_purchase_draft', () => {
  it('calls DELETE /purchases/drafts/{draftId}', async () => {
    ...
    expect(mockMutate).toHaveBeenCalledWith('DELETE', '/companies/test-slug/purchases/drafts/1');
    expect(result.isError).toBeUndefined();
  });
...
describe('fiken_get_purchase_draft_attachments', () => {
  it('calls GET /purchases/drafts/{draftId}/attachments', async () => {
    ...
    mockGet.mockResolvedValue(data);
    const result = await server.getHandler('fiken_get_purchase_draft_attachments')({ draftId: 1 });
  ...
  it('returns error on failure', async () => {
    mockGet.mockRejectedValue(new Error('Fiken 404: Not Found'));
    const result = await server.getHandler('fiken_get_purchase_draft_attachments')({ draftId: 999 });
    expect(result.isError).toBe(true);
  });
});
...
describe('fiken_create_purchase_from_draft', () => {
  it('calls POST /purchases/drafts/{draftId}/createPurchase', async () => {
    mockMutate.mockResolvedValue({ created: true, location: '/companies/test-slug/purchases/5' });
  ...
  it('returns error on failure', async () => {
    mockMutate.mockRejectedValue(new Error('Fiken 400: Bad Request'));
    const result = await server.getHandler('fiken_create_purchase_from_draft')({ draftId: 999 });
    expect(result.isError).toBe(true);
  });
});

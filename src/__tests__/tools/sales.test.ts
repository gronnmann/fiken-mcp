...
describe('fiken_create_sale', () => {
  it('calls POST /sales with body', async () => {
    ...
    const body = {
      date: '2024-01-15',
      lines: [{ description: 'Product', netAmount: 100000, vat: 25000, vatType: 'HIGH', account: '3000' }],
    };
  ...
  it('returns error on failure', async () => {
    ...
    const result = await server.getHandler('fiken_create_sale')({ date: '2024-01-01', lines: [] });
    expect(result.isError).toBe(true);
  });
});
...
describe('fiken_create_sale_draft', () => {
  it('calls POST /sales/drafts with body', async () => {
    mockMutate.mockResolvedValue({ created: true, location: '/companies/test-slug/sales/drafts/1' });
...
describe('fiken_update_sale_draft', () => {
  it('calls PUT /sales/drafts/{draftId} with body (draftId excluded)', async () => {
    ...
    expect(mockMutate).toHaveBeenCalledWith(
      'PUT',
      '/companies/test-slug/sales/drafts/1',
      { date: '2024-02-01', paymentAccount: '1920:10001' },
    );
    expect(result.isError).toBeUndefined();
  });
...
describe('fiken_get_sale_draft_attachments', () => {
  ...
  it('returns error on failure', async () => {
    mockGet.mockRejectedValue(new Error('Fiken 404: Not Found'));
    const result = await server.getHandler('fiken_get_sale_draft_attachments')({ draftId: 999 });
    expect(result.isError).toBe(true);
  });
});

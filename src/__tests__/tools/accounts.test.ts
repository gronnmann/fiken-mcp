...
describe("fiken_list_account_balances", () => {
  ...
  it("returns error on failure", async () => {
    ...
    const result = await server.getHandler("fiken_list_account_balances")({ date: "2024-01-01" });
    expect(result.isError).toBe(true);
  });
});
...
describe("fiken_create_bank_account", () => {
  it("calls POST /bankAccounts with body", async () => {
    ...
    const body = { name: "New Account", bankAccountNumber: "12345678903", type: "normal" as const };

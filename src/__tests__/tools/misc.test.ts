...
describe('fiken_create_product_sales_report', () => {
  it('calls POST /products/salesReport with body', async () => {
    ...
    expect(mockMutate).toHaveBeenCalledWith('POST', '/companies/test-slug/products/salesReport', body);
    expect(result.content[0].text).toBe(JSON.stringify(data, null, 2));
  });

  it('returns error on failure', async () => {
    mockMutate.mockRejectedValue(new Error('Fiken 400: Bad Request'));
    const result = await server.getHandler('fiken_create_product_sales_report')({ from: '2024-01-01', to: '2024-12-31' });
...
describe('fiken_list_time_entries', () => {
  it('calls GET /timeEntries with filters', async () => {
    ...
    mockGet.mockResolvedValue(data);
    const params = { page: 0, pageSize: 50, startDate: '2024-01-01', endDate: '2024-01-31', projectId: 5, userId: 2 };
